...
- ``GenerationBatch.__init__`` — after the standard ``_step()`` has run
  ...
  MTP draft block. Two confirmed tokens are queued for emission and the
  draft block is stashed for the first verify cycle.
...
- ``GenerationBatch.next`` — when the batch holds exactly one MTP-capable
  ...
  multi-token verify forward over ``[next_main] + drafts`` with
  ``n_confirmed=1``. Accepted draft prefixes are emitted, rejections sample
  the mathematically-correct residual, and a new MTP draft block is built
  from the bonus/correction token.

The throughput math (greedy, accept rate p):
  - D1 cost per *cycle*: 1× backbone (2-token verify) + 1× MTP head ≈ 1.15
  - D1 tokens per cycle: 1 + p (accept emits draft+bonus; reject emits correction)
  - D2/D3 can improve this when deeper acceptance stays high; otherwise
    the extra MTP work is intentionally configurable per model.
...
-----------------------------
``cache.trim(n)`` on a ``BatchKVCache`` only updates ``self._idx``; the
...
buffer's ``_size`` by the number of rejected draft positions (mirroring
PR 990's ``prev_tokens = prev_tokens[:-1]`` for D1).
"""
...
def _resolve_mtp_draft_depth(gen_batch: Any) -> int:
    ...
    ``model_settings.mtp_draft_depth`` is attached to the model at load time.
    ``OMLX_MTP_DRAFT_DEPTH`` is an operator override for quick A/B tests
    without touching persisted settings. Depth is intentionally clamped to a
    small range because each extra step adds MTP-cache state that has to be
    rolled back precisely on rejection.
    """
    import os
...
def _restore_or_trim_caches(prompt_cache: List[Any], trim_count: int = 1) -> bool:
    ...
    we trim by ``trim_count``. Layers that support neither cause the entire
    MTP step to fall back to the standard path.
    """
...
def _trim_mtp_cache(mtp_cache: Optional[List[Any]], trim_count: int) -> bool:
    """Trim speculative condition tokens from the MTP-head cache."""
...
def _draft_block_from(
    ...
    *,
    ...
    """Draft up to ``depth`` tokens using recursive native MTP hidden state.

    The first step is the PR-990 D1 path. Additional steps require
    ``mtp_forward(..., return_hidden=True)`` so the MTP head's predicted hidden
    can feed the next draft. Older/unsupported model patches naturally stop at
    D1 by returning logits only.
    """
    import time
...
def _post_init_mtp(gen_batch: Any) -> None:
    ...
    # logprobs come from the standard / patched samplers.
...
def _log_mtp_stats(uid: Any, stats: "_MtpStats", finish_reason: str) -> None:
    ...
    if stats.drafted_tokens > 0:
        ...
        rate_str = "n/a"
    logger.info(
        "MTP[%s] finish=%s tokens=%d cycles=%d depth<=%d "
...
def _run_verify_cycle(gen_batch: Any, state: _MtpState) -> None:
    ...
    # Batched logprobs: one logsumexp over (depth + bonus, vocab).
    ...
    # Reject path: roll target cache and MTP cache back to the committed
    # prefix, replay accepted target tokens when the rejection happened after
    # depth 1, then draft from the correction token.
    t0 = time.perf_counter()
